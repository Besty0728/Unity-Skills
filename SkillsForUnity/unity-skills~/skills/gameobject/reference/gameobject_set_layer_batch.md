...
**Returns**: `{success, totalItems, successCount, failCount, results: [{target, entityId, success, layer}]}`. The layer must already exist in Tags & Layers; an unknown one fails that item (`Layer not found`) and, all-or-nothing, reverts the rest.
