...
So a client that branches on `response.status_code == 200` will read a failed skill call as a success. Always parse the body and check `status` / `errorCode`. The two authorization outcomes (`MODE_RESTRICTED`, `MODE_FORBIDDEN`), `SURFACE_EXCLUDED` and `DRYRUN_REQUIRED` are also HTTP `200` — they are decisions, not transport faults.
...
| `DRYRUN_REQUIRED` | 200 (`status:"error"` in the body) | The user's `dryRunPolicy` (`highRisk` / `allWrites`) gates this write and the call carried no usable `?dryRunToken=`. Nothing ran | Read `details.dryRun` (the preview), then resend the same body with `?dryRunToken=<details.dryRunToken>`; `suggestedFixes[0]` is that exact call. The token is single use for 300 s; `details.reason` says why the presented one was refused. See [operating mode](protocol-operating-mode.md) |
...
| `INSTANCE_MISMATCH` | 409 | The request named the Editor it is meant for — `expectInstance` (exact instanceId) or `expectProject` (productName or project folder name), as a query parameter or an `X-Expect-Instance` / `X-Expect-Project` header — and this server is another one. Checked before anything is queued, so nothing ran. `/health` is never checked | Resend to the instance a `suggestedFixes` entry names (`args.port`, `args.url`). `details.server` says who answered; `details.instances` lists the other registered Editors with `port` and `status` — wait for a `reloading` one rather than switching. `retryStrategy` is `fix_and_retry`. The Python client sends `X-Expect-Instance` itself when it resolved the instance from the working directory or `target`, and follows that instance to a new port |
| `METHOD_NOT_ALLOWED` | 405 | `GET /skill/<name>` on a registered skill: skills only run on `POST` (an unregistered name still answers `NOT_FOUND`). Nothing ran | Resend `details.curl` — the same call as `POST` with the query arguments moved into the JSON body and request-level keys (`mode`, `dryRun`, `diff`, `wire`, `expect*`) kept in the URL. `details.body` (also `suggestedFixes[0].args`) is the converted body |
...
| `MISSING_PACKAGE` | `install_and_retry` | The skill's optional package (ProBuilder / XR / Netcode / DOTween / YooAsset / URP …) is not installed | Tell the user which package is missing (the message names the package id); installing it with `package_install` is their call, then wait out the Domain Reload and retry. A dryRun (`validation.missingPackages`) and recommend (`unavailable`) report the same before any call, so no extra probe is needed |
| `MISSING_PARAM` | `fix_and_retry` | A required parameter was omitted | Add the parameter named in the message. A call rejected by validation ran nothing, and its `details` already carries every validation bucket plus the parameter list (the report a dryRun would give), so fix and retry directly |
| `SEMANTIC_INVALID` | `fix_and_retry` | A parameter was supplied but rejected — out of range, unknown enum value, wrong asset type, or the target already exists | Read the accepted range/values from the message and `details.validation`, correct the args, retry |
