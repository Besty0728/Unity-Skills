...
- **根 `SKILL.md`（round4 重写，round5 定稿）** — round4 围绕短路径重写：路由（执行 / 指导 / 顾问）、写请求带 `expectProject` / `expectInstance`、精确调用形状速查表、一次发现调用（`recommend … topN=3&wire=v2`、多意图、`names=`）、精确的 dryRun 规则、以写后读回为准、`/jobs/<id>?wait=90` 等编译；移出的内容进入新的 `references/protocol-discovery.md`。round5 定稿为 8171 字节（LF 归一化，8192 硬红线）：`/health` 改为会话开始时与第一个调用写在同一条 shell 命令里探测一次，此后只在写被拒、409、503 / 连接被拒或模式行为异常时再探，字段列表加 `dryRunPolicy`（`references/protocol-operating-mode.md` 的 Boot Handshake 同步改写）；dryRun 段补一句：处于 `dryRunPolicy` 之下时用返回的 `?dryRunToken=` 执行；速查表去掉 Edit 标签列（调用行与 "(dryRun first)" 标注全部保留）；读回段写明自己写了 `.cs` 之后调用的 `asset_refresh` 在 `compileTriggered:true` 时同样返回 `waitUrl`；"dryRun 通过才建脚本 → 等编译 → 在一个 batch 里挂组件并设值"写成一条 shell 命令，dryRun 的输出直接交给 `grep -q`、不进上下文，两次调用共用一个请求体变量（原来单独的等待命令并入其中）；更宽视图的四个端点与体积、域重载的括注、与 dryRun 段重复的 "Exact shapes" 句删去，前两项原文都在 references 中；速查示例省去与 `-d` 重复的 `-X POST`，读回段的示例压缩。frontmatter 的 `compatibility` 保留。
