...
- **TODO(M4)：本地自更新双轨 + dryRun 策略设置 UI** — 待 r5-update 合入后补写（Git 轨只做快进与仓库形态守卫、ZIP 轨修复与保留一代备份、游离在发布标签时"切到最新标签"、设置抽屉的 dryRun 策略下拉框、三语本地化键与图集核对）。
- **TODO(M9)：参数说明扩展** — 待 r5-notes 合入后补写（UI / Prefab / Scene / Asset / Animator 中语义不自明的参数，≤ 60 条，全量 schema 增长 ≤ 10 KB，`ParamNoteCoverageTests`）。
...
- **TODO(M7)：元数据一致性** — 待 r5-meta 合入后补写（`RequiresInput` 死项守卫、33 个 URP 存根与真实分支对齐（含 `RequiresPackages`）并加源码守卫、文档类型比对启用表格格式、保留参数在 dryRun 中同样校验类型、`semanticErrors` 同时输出 `field` 与 `parameter`）。
...
- **TODO(lead)：集成验证** — 全量 EditMode、v1 契约比对、`/skillcheck`（技能数 805 不变）、`/metacheck`、根文档字节、活体行为检查的结果待集成 2 后补写。
