...
通过 REST API 让 AI 直接控制 Unity 编辑器。739 个 REST Skills + 20 个 Advisory 模块。
...
│   │   │   └── *Skills.cs × 51           # 功能模块 (共 739 Skills)
...
- **Window 菜单单入口（用户拍板）**：`[MenuItem("Window/UnitySkills")]` 是叶子项且全项目唯一——Unity 里叶子与 `Window/UnitySkills/...` 子菜单不能并存（叶子会被吞）。**禁止**在该前缀下新增任何 `[MenuItem]`；二级面板（宏录制、审计日志）只走面板内按钮 + 快捷键。
- **面板快捷键**：命令统一注册在 `Editor/UI/ShortcutActions.cs`（Unity `[Shortcut]` 特性，出厂默认键取 Alt+Shift+字母 空闲区：主面板 U / 审计 L / 宏录制 M）。新增可绑定面板 = 加一个 `[Shortcut]` 静态方法 + `Commands` 清单追加一条，设置抽屉 Shortcuts 节自动枚举；绑定持久化由 ShortcutManager profile 自管，不写 EditorPrefs。
...
**REST 端点**：`/permission/{status,grant,approve,deny,revoke,allowlist,allowlist/{add,remove},audit}`；`/health` 含 `currentMode` / `panelApprovalRequired` / `pendingCount`；`/skills` 每条含 `mode`（`"semi"`/`"full"`）与 `approvalBehavior`（`"allow"`/`"grant"`/`"forbid"`，Approval 模式下的预期行为，忽略 Allowlist/one-shot 状态）字段；`POST /skills/batch`（v2.1.0+）跨技能聚合执行——每步走完整单技能管线（含权限门，授权类错误中断透传 grant token），默认 fail-fast，`?mode=dryRun` 整序列预演；v2 新增步间 `{"$ref":"$N.path"}` 引用（后步取前步 unwrap 后 result 的 `SelectToken`，解析失败该步 `SEMANTIC_INVALID`）与 `?mode=transactional`（全有或全无，任一步失败经 `Undo.RevertAllDownToGroup` 回滚已执行步，顶层 `status:"rolled_back"`；MutatesAssets 步标 `rollbackReliability:"partial"`）。写型 `POST /skill/{name}?diff=1` 成功响应附顶层 `sceneDiff`（changed/added/removed 对象的 before/after 语义差异，path 带类型前缀，上限 20 对象×50 条，batch 不支持）。观测类只读端点：`GET /compile/status`（上次编译结果 `{success,errorCount,errors:[{file,line,message,assembly}],...}`，经 SessionState 跨 domain reload 存活）、`GET /events`（长轮询事件通道，500 条内存环形缓冲，`since`/`cursor` 续拉，含 `compilation_finished` / `server_restored`（payload 带 `lastCompilation` 作重连锚点）/ `console_error` 等事件，`seq` 跨 reload 单调不回退）、`GET /analytics?window=1h|24h|7d|all`（执行遥测聚合 summary / topSkills / errorProneSkills / slowestSkills / byMode / byAgent，数据源 `Library/UnitySkillsTelemetry.jsonl`）。
...
- **新增 UI 窗口**：在 `Editor/UI/` 加 `.cs + .uxml + .uss` 三件套（参考 `UnitySkillsWindow` / `UnitySkillsAuditWindow` / `MacroRecorderWindow`）。
